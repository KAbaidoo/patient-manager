...
app.post('/signin', (req, res)=>{
    ...
    (async()=>{
       await  Users.findOne(data, (err,results)=>{
            if (!results) {
                res.render('signin', {title: 'Sign in page', message: {msg:'Wrong login credentials', class:'alert-danger'}});
                console.log(err);
            } else {
                res.render('index', {title: 'Home',message: {msg:'Login successful!',class:'alert-success'}})
            }
        })
    })()
...
app.post('/signup',[check('fname','First name cannot be empty').notEmpty(),
...
check('password','Choose a password').notEmpty()
    ...
    if (!errors.isEmpty()) {
        res.render('add_user', {title:'Add user', errors: errors.array(), message:{}});
    } else {
        let user = new Users();
        ...
        user.save((err)=>{
            if (err) {
               ...
               res.render('signin', { title: 'Sign in page', message : {msg:'User saved succesfully, You may now Login ',class:'alert-success'}});
            }
        });
    };
});
...
// Add Submit POST Route
app.post('/patients/add',[check('fname','First name cannot be empty').notEmpty(),
check('lname','Last name cannot be empty').notEmpty(),
check('gender','Select a gender').notEmpty(),
check('birthdate','Select birthdate').notEmpty(),
check('address','Enter address').notEmpty(),
check('phone','provide phone nuber').notEmpty()], (req,res)=>{
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        res.render('add_patient', {title:'Add Patient', errors: errors.array(),message:{}});
    } else {
...
app.get('/patients', (req,res)=>{
    Patients.find({}, (err, patients)=>{
        ...
        res.render('patients', {title:'Manage patients', patients: patients ,message:{} });
        }
    })
...
app.get('/patients/:id',(req,res)=>{
    Patients.findById(req.params.id, (err,patient)=>{
        ...
        res.render('patient_details', {
            patient: patient, title: 'Patient info',message:[]
        })
        };
    });
});
...
app.get('/patients/edit/:id',(req,res)=>{
    Patients.findById(req.params.id, (err,patient)=>{
        ...
        res.render('edit_patient', {
            patient: patient, title: 'Edit patient info',message:{}
        })
        };
    });
});
...
app.get('/patients/payments/:id', (req,res)=>{
    Patients.findById(req.params.id, (err,patient)=>{
        if (err) {
            ...
            res.render('payments',{title: 'Payments', patient: patient, message:{}});
        };
    });
